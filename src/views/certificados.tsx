  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-foreground">Certificados de Avance</h2>
      ...
      <Card className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div>
            <label className="text-xs text-muted-foreground font-medium">Gremio</label>
          ...
          <div>
            ...
            <input type="date" value={periodStart} onChange={e => setPeriodStart(e.target.value)} className="mt-1 w-full h-9 px-3 border border-border rounded-md text-sm" />
          </div>
          <div>
            <label className="text-xs text-muted-foreground font-medium">Período hasta</label>
            <input type="date" value={periodEnd} onChange={e => setPeriodEnd(e.target.value)} className="mt-1 w-full h-9 px-3 border border-border rounded-md text-sm" />
          </div>
      ...
      {guildTask && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <Card><CardContent className="p-4">
            <div className="text-xs text-muted-foreground">Gremio</div>
            <div className="text-lg font-semibold mt-1">{effectiveGuild}</div>
            <div className="text-[11px] text-muted-foreground mt-0.5">Tarea: {guildTask.name}</div>
          </CardContent></Card>
          <Card><CardContent className="p-4">
            <div className="text-xs text-muted-foreground">Monto total del gremio</div>
            <div className="text-lg font-semibold mt-1">{formatCurrency(totalGuild)}</div>
            <div className="text-[11px] text-muted-foreground mt-0.5">Presupuestado</div>
          </CardContent></Card>
          <Card><CardContent className="p-4">
            <div className="text-xs text-muted-foreground">% Repercusión distribuido</div>
            <div className="text-lg font-semibold mt-1">{totalRepercussion}%</div>
            <div className="text-[11px] text-muted-foreground mt-0.5">{certificateLines.length} subtareas certificables</div>
          </CardContent></Card>
          <Card><CardContent className="p-4">
            <div className="text-xs text-muted-foreground">Monto a pagar este período</div>
            <div className="text-lg font-semibold mt-1 text-emerald-700">{formatCurrency(totalToPay)}</div>
            <div className="text-[11px] text-muted-foreground mt-0.5">{format(parseISO(periodStart), "dd MMM", { locale: es })} - {format(parseISO(periodEnd), "dd MMM", { locale: es })}</div>
          </CardContent></Card>
        </div>
      )}
          ...
          {certificateLines.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="border-b border-border text-[11px] uppercase text-muted-foreground font-semibold">
                  <tr>
                ...
                <tbody>
                  {certificateLines.map(line => (
                    <tr key={line.subtareaId} className="border-b border-border/50 hover:bg-muted/30">
                      <td className="py-2 px-2">
                        <button onClick={() => openTaskModal(line.subtareaId)} className="text-xs font-medium text-foreground hover:text-primary flex items-center gap-1">
                      ...
                      <td className="py-2 px-2 text-center text-xs text-muted-foreground">{line.previousProgress}%</td>
                      <td className="py-2 px-2 text-center">
                      ...
                      <td className="py-2 px-2 text-center">
                        <Badge className={cn('text-[10px]', line.periodProgress > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground')}>
                          +{line.periodProgress}%
                        </Badge>
                      </td>
                      <td className="py-2 px-2 text-right text-xs text-foreground">{formatCurrency(line.totalAmount)}</td>
                      <td className="py-2 px-2 text-right text-sm font-semibold text-emerald-700">{formatCurrency(line.amountToPay)}</td>
                    </tr>
                  ))}
                  <tr className="bg-muted/30 font-semibold">
                    <td colSpan={5} className="py-3 px-2 text-right text-xs uppercase text-muted-foreground">Total a pagar</td>
                    <td className="py-3 px-2 text-right text-xs text-muted-foreground">{formatCurrency(certificateLines.reduce((s, l) => s + l.totalAmount, 0))}</td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <Users className="w-4 h-4 text-muted-foreground" /> Gremios de la obra
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
                ...
                return (
                  <button
                    ...
                    className={cn('w-full flex items-center gap-3 p-2.5 rounded-md border transition text-left',
                      g === effectiveGuild ? 'border-orange-300 bg-primary/10' : 'border-border hover:bg-muted/30')}
                  >
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-muted-foreground" /> Resumen de certificados previos
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-2.5 rounded-md border border-border">
                  <div>
                    <div className="text-xs font-medium text-foreground">{cert.period}</div>
                    <div className="text-[11px] text-muted-foreground mt-0.5">Certificado de avance</div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-semibold text-foreground">{formatCurrency(cert.amount)}</div>
